...
import {getFaceVerifyResult,exportResult} from 'actions/review';
...
class Result extends React.Component {
    ...
    onStart = (tableParams) => {
        ...
        return getFaceVerifyResult(_params).then(e => {
            const state = { list: e.data.this_page ? e.data.this_page : [], total: e.data.total };

            if(this.state.type === 'time'){
                state.exportUrl = exportResult('/face_verify_xlsx_result',_params);
            }else{
                state.exportUrl = '';
            }
            this.setState(state);
    ...
    render() {
            ...
            >
                ...
                >
                    <Select value={this.state.product} style={{ width: 150 }} onChange={this.onProductChange}>
                        ...
                        <Option value={SERVICE_ID.FACE_VERIFY+''}>动作活体SDK</Option>
                        <Option value={SERVICE_ID.LITE_VERIFY+''}>H5/小程序</Option>
                    </Select>
        ...
        if(info){
            ...
            if(info.service_id !== SERVICE_ID.LITE_VERIFY+''){
                otherInfo = [
...
// WEBPACK FOOTER //
// ./src/pages/ProductSc/Result/index.js